	...
	namespace Input
	{
		...
		void Service(HRAWINPUT i_Input)
		{
			...
			if (bytesData)
			{
				...
				ret = GetRawInputData(i_Input, RID_INPUT, m_pReadBuffer, &bytesData, sizeof(RAWINPUTHEADER));// return value of this function is number of bytes copied in m_pReadBuffer if func is successfull
				assert(ret == bytesData); // check for the same

				RAWINPUT * pRawInput = reinterpret_cast<RAWINPUT *>(m_pReadBuffer);

				if (pRawInput->header.dwType == RIM_TYPEKEYBOARD) // if data comes from keyboard
				{
					if (pRawInput->data.keyboard.Flags == RI_KEY_BREAK)// if the key is up
					{ // up
					...
					else if (pRawInput->data.keyboard.Flags == RI_KEY_MAKE)// if the key is down
					{ // down
						...
						if (bChanged)
						{
							if (m_pKeyStateChangeCallback)
							{
								...
								key = pRawInput->data.keyboard.VKey;
							}
